 ...
 * =============================================================================
 * 
 * Modal for editing an existing finding
 * 
 * - QM, Lead Auditor, and Auditor can edit findings
 * - Can only edit when status = 'open'
 */

import { useState, useEffect } from 'react';
import { Edit, AlertTriangle, FileText, AlertCircle, Clipboard } from 'lucide-react';
import Modal from '@/components/ui/Modal';
import Button from '@/components/ui/Button';
import Spinner from '@/components/ui/Spinner';
import SearchableSelect, { SelectOption } from '@/components/ui/SearchableSelect';
import { editFinding, Finding } from '@/lib/api/findings';
...
type EditFindingFormData = z.infer<typeof editFindingSchema>;
...
export default function EditFindingModal({
  ...
  } = useForm<EditFindingFormData>({
    resolver: zodResolver(editFindingSchema),
    defaultValues: {
      severity: finding.severity,
      requirementId: finding.requirement?.id || 0,
      processId: finding.process?.id || 0,
  ...
  /**
   * Reset form with finding data when finding changes
   */
  useEffect(() => {
    if (isOpen && finding) {
      reset({
   ...
   */
  const onSubmit = async (data: EditFindingFormData) => {
    try {
      setApiError(null);

      await editFinding(auditId, finding.id, {
      ...
      setApiError(error.response?.data?.message || 'Failed to update finding');
    }
  };
